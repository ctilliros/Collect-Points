...
def sample_job_every_1000s():    
	...
	poi_gdf = ox.pois_from_place(place=place, which_result=2)
	poi_gdf = poi_gdf[poi_gdf.name.notnull()]
	poi_gdf.amenity = poi_gdf.amenity.fillna('other')

	dfosmnx = pd.DataFrame(columns={"title","source",'category', "subcategory", "description","latitude", "longitude", "date_add"})
	...
	for key, value in poi_gdf.iterrows():
		...
		sql = 'select * from points_of_interest where title =%s and latitude =%s and longitude=%s;'
		cursor.execute(sql,(title, latitude, longitude,))
		sql_values = cursor.fetchall()
		conn.commit()
		dfopendata=dfopendata.append({"title":title,"source":1,'category':category, "subcategory":subcategory, "description":description,"latitude":latitude, "longitude":longitude, "date_add":date_now},ignore_index=True)
		if not sql_values:        
			sql = 'insert into points_of_interest(title, source,category, subcategory, description,latitude, longitude, date_add)\
			 values (%s, %s, %s,%s, %s,%s,%s,%s)'
			cursor.execute(sql, (title, source, category, subcategory, description, latitude, longitude, date_now,))
			conn.commit()
	...
	for post in feed.entries:
		...
		if not sql_values and latitude != 0 and longitude != 0:        
			...
			sql = 'insert into points_of_interest(title, source,category, subcategory, description,latitude, longitude, date_add)\
			 values (%s, %s, %s,%s, %s,%s,%s,%s)'
			cursor.execute(sql, (title, 0, category, subcategory, description, latitude, longitude, date_now,))
			conn.commit()
